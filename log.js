  ...
  reader.onload = function(ev) {
    try {
      ...
      for (var i = 1; i < lines.length; i++) {
        ...
        var planned = iPlanned >= 0 ? (parseInt(parts[iPlanned]) || actual) : actual;
...
function parseDateToTimestamp(dateStr, timeStr) {
  ...
  var timeNormalized = (timeStr || '00:00').trim();
  
  // Try ISO format first (YYYY-MM-DD)
  var isoDate = normalized.match(/^(\d{4})-(\d{2})-(\d{2})$/);
  if (isoDate) {
    var timeParts = timeNormalized.split(':');
  ...
  // Try DD/MM/YYYY or DD.MM.YYYY (European formats)
  var euroDate = normalized.match(/^(\d{1,2})[\/\.](\d{1,2})[\/\.](\d{4})$/);
  if (euroDate) {
    var timeParts = timeNormalized.split(':');
    var hours = parseInt(timeParts[0]) || 0;
    var mins = parseInt(timeParts[1]) || 0;
  ...
  var usDate = normalized.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/);
  if (usDate) {
    var timeParts = timeNormalized.split(':');
    var hours = parseInt(timeParts[0]) || 0;
    var mins = parseInt(timeParts[1]) || 0;
  ...
  var parsed = Date.parse(normalized + ' ' + timeNormalized);
